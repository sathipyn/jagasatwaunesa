    ...
    <div class="min-h-screen flex">
        ...
        <div class="w-full lg:w-[56%] flex items-center justify-center p-8 lg:px-10 xl:px-14">
            <div class="w-full max-w-lg">
                ...
                @if(session('status'))
                    <div class="bg-green-50 text-green-700 p-4 rounded-xl mb-6 text-sm border border-green-200">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login.store', absolute: false) }}" class="space-y-5">
                    ...
                    <div>
                        <div class="mb-1.5 flex items-center justify-between gap-3">
                            ...
                            <a href="{{ route('password.request') }}" class="text-sm font-semibold text-pink-500 hover:underline">
                                Lupa password?
                            </a>
                        </div>
                ...
                <p class="text-center text-sm text-gray-500 mt-8">
                    Belum punya akun?
                    <a href="{{ route('register', request()->filled('redirect_to') ? ['redirect_to' => request('redirect_to')] : []) }}" class="text-pink-500 font-semibold hover:underline">Sign Up</a>
                </p>
            </div>
        </div>
    </div>
